...
@section('title', 'Přidat firmu - CRM System')
...
<div class="space-y-6">
    ...
    <div class="flex justify-between items-center">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Přidat novou firmu</h1>
            <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">Vyplňte informace o nové firmě</p>
        </div>
        ...
        <form method="POST" action="{{ route('companies.store') }}" class="space-y-6">
                    ...
                    @foreach(\App\Models\Company::getAvailableCategories() as $key => $category)
                        <div class="relative">
                            <input type="radio" 
                                   ...
                                   {{ old('category', 'other') === $key ? 'checked' : '' }}
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <textarea name="address" id="address" rows="3"
                              ...
                              placeholder="Ulice, město, PSČ">{{ old('address') }}</textarea>
                    @error('address')
                        <p class="form-error">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div>
                ...
                <textarea name="notes" id="notes" rows="4"
                          class="form-input resize-none" 
                          placeholder="Dodatečné informace o firmě...">{{ old('notes') }}</textarea>
                @error('notes')
                    <p class="form-error">{{ $message }}</p>
                @enderror
            </div>
